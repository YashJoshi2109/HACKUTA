...
function getDropper() {
  ...
  for (let i = 1; i <= partyCount; i++) {
    $("#partyNamed").html("");
    VoteTrackerContract.methods.getNames(i).call((error, response) => {
      if (error) {
        console.log(error);
      } else {
...
function generateVote() {
  ...
  VoteTrackerContract.methods
    .generateVote(account0, partyNamed, adhaar, "Mumbai")
    .send()
    .then((result) => {
      if (result.status === true) {
        alert("Success");
...
function getPartyCount() {
  VoteTrackerContract.methods.getPartyCount().call((error, response) => {
    if (error) {
      console.log(error);
    } else {
      partyCount = response;
...
function verify() {
  ...
  VoteTrackerContract.methods
    .registerUser(account0, adhar, email)
    .send()
    .then((result) => {
      if (result.status === true) {
        alert("Success");
...
function adminLogin() {
  var key;
  VoteTrackerContract.methods.getAdminKey().call((error, response) => {
    if (error) {
      console.log(error);
    } else {
      key = response;
